...
#--------------------------------------------------------------#
# Step 2: Fetch MCP251XFD sources
#--------------------------------------------------------------#
say "STEP2" "Fetching MCP251XFD sources"

BASE_URL="https://raw.githubusercontent.com/raspberrypi/linux/${KERNEL_BRANCH}/drivers/net/can/spi/mcp251xfd"
...
for f in \
    ...
    mcp251xfd.h
do
    wget -q -O "$MODULE_SRC_DIR/$f" "$BASE_URL/$f"
done
...
for f in \
    ...
    Makefile
do
    cp "$MODULE_SRC_DIR/$f" "$MODULE_BUILD_DIR/"
done
